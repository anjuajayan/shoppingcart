...
<head>
	<title>Magzworld | Articles</title>
...
<?php
		...
		$mg="select * from mag where category='Articles'";
		$ex=mysqli_query($cf,$mg);
		while($fetch=mysqli_fetch_array($ex))
		{
	?>				
			       
	<div class="card"><form name="m" method="post" enctype="multipart/form-data" action="articles.php?name=<?php echo $fetch['name'] ?>">
	<h5 style="text-align:center"><b><?php echo $fetch['name']; ?></b></h5>
	
  <img class="cover" src="../admin/productimage/articles/<?php echo $fetch['image'] ?>" alt="image" style="width: 250px;height: 300px;align-self: center;">
 ...
 {
 	$a=$_GET['name'];
 $mn="select * from mag where name='$a'";
		$exx=mysqli_query($cf,$mn);
 ?>
 <table class="table">
 	<th><?php echo $fetch['name'] ?></th>
 	<tr><?php echo $fetch['det'] ?></tr>
 </table>
<?php } ?>
